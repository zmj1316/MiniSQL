...
u32 btree_delete(const char* idxname, Rule* rule)
{
    ...
    size_t i;
    /* locate */
    for (i = 0; i < nd.N && cmp(bt.type, nd.datas[i], rule->target) <= 0; i++);
    if (i==0)
    {
...
void mergeNode(btree*, node* source, node* dst)
{
